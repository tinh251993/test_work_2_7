...
void NavigationURLLoaderImpl::OnReceiveResponse(
  ...
  CRMC_BrowserIF* m_brwIF = CRMC_BrowserIF::getInstance();

  if (m_brwIF){
    std::string url_str = url_.spec();
    DVLOG(0) << " URL loader = " + url_str;
    m_brwIF->CheckForUpdate();
    DVLOG(0)<<"Check for update";
   
  }else{
    DVLOG(0) << "co van de voi browserIF";
  } 
