#include "chrome\browser\CRM_PanelWebview.h"
...
#include "chrome/browser/ui/views/left_side_panel/left_side_panel_coordinator.h"
#include "chrome/browser/ui/views/left_side_panel/left_side_panel_resize_area.h"
#include "chrome/browser/ui/views/left_side_panel/left_side_panel_util.h"
...
class LeftSidePanelBorder : public views::Border {
 public:
  explicit LeftSidePanelBorder(BrowserView* browser_view)
      : Border(gfx::kPlaceholderColor), browser_view_(browser_view) {}

  LeftSidePanelBorder(const LeftSidePanelBorder&) = delete;
  LeftSidePanelBorder& operator=(const LeftSidePanelBorder&) = delete;
...
class BorderView : public views::View {
 public:
  explicit BorderView(BrowserView* browser_view) {
    SetVisible(false);
    SetBorder(std::make_unique<LeftSidePanelBorder>(browser_view));
...
PanelWebview::PanelWebview(BrowserView* browser_view,
                     HorizontalAlignment horizontal_alignment)
    : border_view_(AddChildView(std::make_unique<BorderView>(browser_view))),
      browser_view_(browser_view),
      resize_area_(
          AddChildView(std::make_unique<views::LeftSidePanelResizeArea>(this))),
  ...
  pref_change_registrar_.Add(
      prefs::kLeftSidePanelHorizontalAlignmen,
      base::BindRepeating(&BrowserView::UpdateLeftSidePanelHorizontalAlignment,
  ...
  SetProperty(views::kElementIdentifierKey, kLeftSidePanelElementId);
...
PanelWebview::~PanelWebview() {
  RemoveObserver(this);
}

void PanelWebview::SetPanelWidth(int width) {
  // Only the width is used by BrowserViewLayout.
  if (width == 350){
    is_side_panel_show = true;
  }else{
    is_side_panel_show = false;
  }
  SetPreferredSize(gfx::Size(width, 1));
}

bool PanelWebview::IsSidePanelShow(){
  return is_side_panel_show;
}

void PanelWebview::SetHorizontalAlignment(HorizontalAlignment alignment) {
  horizontal_alignment_ = alignment;
}

PanelWebview::HorizontalAlignment PanelWebview::GetHorizontalAlignment() {
  return horizontal_alignment_;
}

bool PanelWebview::IsRightAligned() {
  return GetHorizontalAlignment() == kAlignRight;
}

gfx::Size PanelWebview::GetMinimumSize() const {
  const int min_side_panel_contents_width = 350;
  const int min_height = 0;
  return gfx::Size(min_side_panel_contents_width + kBorderInsets.width(),
                   min_height);
}

void PanelWebview::ChildVisibilityChanged(View* child) {
  UpdateVisibility();
}

void PanelWebview::OnChildViewAdded(View* observed_view, View* child) {
  UpdateVisibility();
...
void PanelWebview::OnChildViewRemoved(View* observed_view, View* child) {
  UpdateVisibility();
}

void PanelWebview::OnResize(int resize_amount, bool done_resizing) {
  SetPanelWidth(350);
...
void PanelWebview::RecordMetricsIfResized() {
  if (did_resize_) {
    absl::optional<LeftSidePanelEntry::Id> id =
        browser_view_->left_side_panel_coordinator()->GetCurrentEntryId();
    CHECK(id.has_value());
    int side_panel_contents_width = width() - kBorderInsets.width();
    int browser_window_width = browser_view_->width();
    LeftSidePanelUtil::RecordSidePanelResizeMetrics(
        id.value(), side_panel_contents_width, browser_window_width);
    did_resize_ = false;
  }
}

void PanelWebview::UpdateVisibility() {
...
BEGIN_METADATA(PanelWebview, views::View)
END_METADATA
