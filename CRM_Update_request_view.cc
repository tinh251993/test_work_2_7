...
bool CRMC_UpdateRequestView::is_show = false;
namespace{
...
class CRMC_UpdateRequestDelegate : public ui::DialogModelDelegate {
 public:
  ...
  void OnWindowClosing() {
        DVLOG(0) << "Current do nothing 1";
        CRMC_UpdateRequestView::is_show = false;
  }
...
class CRMC_UpdateRequestView::BrowserRemovalObserver
...
// static
void CRMC_UpdateRequestView::Show(Browser* browser, std::string version) {
...
//     offer_uma_optin = !IsMetricsReportingPolicyManaged();
  auto browser_observer =
      std::make_unique<CRMC_UpdateRequestView::BrowserRemovalObserver>(
          browser);

  if (browser && (browser->tab_strip_model()->GetActiveWebContents())) {
    ShowBubble(browser, version);
    return;
  }
}

// static
views::BubbleDialogDelegate* CRMC_UpdateRequestView::GetInstance() {
  return g_instance_for_test;
}

views::BubbleDialogDelegate* CRMC_UpdateRequestView::ShowBubble(
    Browser* browser, std::string version) {
  ...
  // bubble_delegate->CloseIfNeed();
  // std::string label_message = "Update version " + version;
  // std::u16string label;
  // base::UTF8ToUTF16(label_message.c_str(), label_message.size(), &label);


  ui::DialogModel::Builder dialog_builder(std::move(bubble_delegate_unique));

  dialog_builder
      .SetTitle(u"Update dialog")
      .DisableCloseOnDeactivate()
      .SetIsAlertDialog()
      .SetDialogDestroyingCallback(
          base::BindOnce(&CRMC_UpdateRequestDelegate::OnWindowClosing,
                         base::Unretained(bubble_delegate)))
      .AddParagraph(ui::DialogModelLabel(u"Update version "));
  ...
  CRMC_UpdateRequestView::is_show = true;
  return bubble_ptr;
}
