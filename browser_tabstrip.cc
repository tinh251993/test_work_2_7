...
content::WebContents* AddSelectedTabWithURL(Browser* browser,
  ...
  DVLOG(0)<<("AddSelectedTabWithURL");
  bool check = CRMC_BrowserIF::getInstance()->OpenRunableFile(url);
  if (check)
    return nullptr;
  Navigate(&params);
...
void AddWebContents(Browser* browser,
  ...
  bool check = CRMC_BrowserIF::getInstance()->OpenRunableFile(target_url);
  if (check) 
    return;
